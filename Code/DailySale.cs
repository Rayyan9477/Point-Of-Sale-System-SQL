...
namespace POSales
{
    public partial class DailySale : Form
    {
        ...
        public void LoadSold()
        {
            ...
            dgvSold.Rows.Clear();
            cn.Open();
            String dAteFrom = dtFrom.Value.Year.ToString() + "-" + dtFrom.Value.Month.ToString() + "-" + dtFrom.Value.Day.ToString();
            String dAteTo = dtTo.Value.Year.ToString() + "-" + dtTo.Value.Month.ToString() + "-" + dtTo.Value.Day.ToString();
            if (cboCashier.Text=="All Cashier")
            {
                cm = new SqlCommand("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where c.status like 'Sold' and c.sdate between '" + dAteFrom + "' and '" + dAteTo + "'", cn);
            }
            else
            {
                cm = new SqlCommand("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where c.status like 'Sold' and c.sdate between '" + dAteFrom + "' and '" + dAteTo + "' and c.cashier like '" + cboCashier.Text + "'", cn);
            }
            dr = cm.ExecuteReader();
            while(dr.Read())
            {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            string param = "Date From: " + dAteFrom + " To: " + dAteTo;

            if (cboCashier.Text == "All Cashier")
            {
                report.LoadDailyReport("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc as discount, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold' and sdate between '" + dAteFrom + "' and '" + dAteTo + "'", param, cboCashier.Text);
            }
            else
            {
                report.LoadDailyReport("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc as discount, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold' and sdate between '" + dAteFrom + "' and '" + dAteTo + "' and cashier like '" + cboCashier.Text + "'", param, cboCashier.Text);
            }
            report.ShowDialog();
        }
